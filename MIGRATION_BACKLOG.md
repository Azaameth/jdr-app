...
Feature-parity gap between `legacy-reference/index.html` (the pre-Vue monolith) and the current app, as of 2026-07-21. Intended to be tackled one item at a time — not a commitment to build all of this at once.

**This file is the source of truth for migration status.** `legacy-reference/README.md` only points at where the legacy spec for each feature lives — check here for what's actually shipped.
...
- **Inventory slots & "dons" (gifts) system** — categorized backpack slots (10 fixed categories, each capped, empty slots computed client-side, never stored), typed weapons/armor lists padded to a minimum of 3 slots, and a dons list with a mana-cost/damage-dice/flavor-text detail modal. Shipped via spec-kitty mission `inventory-slots-dons-01KXRF5M`; UI in `src/components/BackpackGrid.vue`, `WeaponArmorList.vue`, `InventorySlotModal.vue`, `DonList.vue`, `DonDetailModal.vue` (wired into `PlayerView.vue`). **Schema note**: the original typed schema (`src/models/types/Inventory.ts` — `InventoryCategory`, `InventoryItem`, `WeaponArmorItem`, `CharacterInventory`) and its store/repository (`useInventoryStore`, `InventoryRepository`) were replaced by the unified `GearEntry` schema in commit `86f6210` ("unifier equipement/inventaire sur le schema GearEntry — Cluster 4"); current code lives in `src/models/repositories/EquipmentRepository.ts`, `src/models/repositories/ItemRepository.ts`, and `src/controllers/useEquipmentStore.ts`. See `docs/rpg-data-model.md` for the current schema.
...
Legacy pointers and current status for each; the "open question" is what still needs deciding before implementation starts.
