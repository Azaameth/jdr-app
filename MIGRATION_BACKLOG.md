...
- **Cluster 0 — operational bug fixes.** Done (see "Bugs" section above) — `seed:static:admin` alias added, `seed:defaultchars:admin` run against the live project. Still pending: running `seed:static:admin` itself against the live project to push the new Cosmology images and confirm Factions/Cosmology data is current.
- **Cluster 1 — schema-first features.** Equipment stat effects, remaining transformation-generalization work (attaching a transformation to an arbitrary character — no guided UI exists yet, only seed scripts / the raw-JSON editor). Do these before pages that might depend on them (e.g. Create a Character).
