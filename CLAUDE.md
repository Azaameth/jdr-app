...
Node version is pinned via `.node-version` (fnm/nvm-compatible); CI matches — `.github/workflows/ci.yml` runs type-check/lint/unit/e2e on PRs and `dev`/`main` pushes, `.github/workflows/deploy.yml` builds and deploys to GitHub Pages on push to `main`.
